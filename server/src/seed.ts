...
const getRandomImage = (category: string, index: number) => {
  ...
  return [
    baseImages[category as keyof typeof baseImages],
    baseImages[category as keyof typeof baseImages].replace('?w=400&h=400', '?w=800&h=600'),
    baseImages[category as keyof typeof baseImages].replace('?w=400&h=400', '?w=600&h=400'),
  ];
};

const getRandomSpecs = (category: string) => {
  const specs = {
    electronics: {
...
async function main() {
  ...
  for (let row = 0; row < 10; row++) {
    for (let col = 0; col < 4; col++) {
      ...
      const categoryProducts = productTitles[category as keyof typeof productTitles];
